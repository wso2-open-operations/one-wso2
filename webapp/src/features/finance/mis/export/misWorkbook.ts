...
//
// The split is also what ticket 18 extends rather than forks: Flash describes
// its own sheets and hands them to this same writer.
...
export const MIS_NUMBER_FORMATS = {
  ...
  COUNT: "#,##0",
  /**
   * A percentage as MIS reports one, which is NOT Excel's `0.00%` — and which
   * is deliberately the SAME string as `CURRENCY` above.
   *
   * The ARR backend sends 98.25 to mean 98.25%, and `formatMisValue` renders it
   * as a bare number with two decimals and no sign — identically to money.
   * Excel's percent format multiplies by 100 on display, so `0.00%` here would
   * put 9,825% on the screen. The row's own label is what says it is a
   * percentage, on the sheet exactly as on screen.
   *
   * Named separately anyway, because the two are the same VALUE and not the
   * same FACT: spec §3 draws a line between them that Scale depends on, and a
   * percentage row sharing `CURRENCY` by reference would lose the distinction
   * the moment either wanted to move.
   */
  PERCENTAGE: "#,##0.00",
} as const;
...
export async function writeMisWorkbook(spec: MisWorkbookSpec): Promise<ArrayBuffer> {
  ...
  for (const sheet of spec.sheets) {
    ...
    for (const row of sheet.rows) {
      ...
      // bare `= { bold: true }` would be the line ticket 18 has to EDIT when
      // Flash wants its title at size 16 (`generateAnnualSheet.js`) rather than
      // a field it can add beside this one.
