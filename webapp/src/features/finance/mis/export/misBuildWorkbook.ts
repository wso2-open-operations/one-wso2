...
//
// ---- and what ticket 18 extends --------------------------------------------
//
// Flash's workbook is the same three moves: a caption, header rows, then data
// rows carrying `bold` and an `outlineLevel`. It describes its own sheets and
// hands them to the same `writeMisWorkbook`. What it will add is merged title
// rows (`generateAnnualSheet.js` merges A1:H1), and a merge list is additive to
// `MisWorkbookSheet` — a field this builder never sets — rather than a fork.
...
function figureCell(raw: MisRawValue, valueType: MisValueType): MisWorkbookCell {
  ...
  return { value: raw, numFmt: NUMBER_FORMAT_BY_VALUE_TYPE[valueType] };
}
